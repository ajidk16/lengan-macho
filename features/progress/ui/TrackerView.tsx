...
import { useBodyLogs, useCreateBodyLog } from "../queries/progress.queries";
...
export function TrackerView() {
  ...
  const [armCm, setArm] = useState("");
  const { mutate, isPending } = useCreateBodyLog();

  const { data, isLoading } = useBodyLogs("1");
  const selectedDate = useProgressStore((s) => s.selectedDate);
      ...
      {latestEntry && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard
            ...
            change={
              previousEntry
                ? getChange(latestEntry.weight, previousEntry.weight)
                : undefined
            }
          ...
          <StatCard
            ...
            change={
              previousEntry
                ? getChange(latestEntry.armSize, previousEntry.armSize)
                : undefined
            }
            ...
            <StatCard
              ...
              change={
                previousEntry?.chestSize
                  ? getChange(latestEntry.chestSize, previousEntry.chestSize)
                  : undefined
              }
            ...
            <StatCard
              ...
              change={
                previousEntry?.waistSize
                  ? getChange(latestEntry.waistSize, previousEntry.waistSize)
                  : undefined
              }
      ...
      <BrandCard title={`📋 ${t("progressHistory")}`} variant="gradient">
        <div className="overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow
                className={isDarkMode ? "border-gray-700" : "border-orange-200"}
              >
                <TableHead
